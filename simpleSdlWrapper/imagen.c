...
int irudiaKargatu(char *fileName)
{
  ...
  SDL_Surface* surface;
		...
		if (surface == NULL )
    {
      ...
      irudiak[irudiKop].texture = SDL_CreateTextureFromSurface(getRenderer(), surface);
	...
	{
		printf("Has superado el maixmo de Imagenes por aplicaci�n. Para aumentar imagen.h\n");
		return -1;
	}
